...
export default function VerificarRenaper({ verificacionToken, onSuccess }: Props) {
  ...
  function handleVerificar() {
    ...
    startTransition(async () => {
      setStage("procesando");
      const res = await fetch("/api/voter/verificar-renaper", {
        method: "POST",
      ...
      <div className="flex flex-col gap-5">
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-800 flex flex-col gap-2">
          ...
          <ol className="list-decimal list-inside space-y-1 text-xs">
            ...
            <li>El sistema compara biométricamente ambas imágenes</li>
          </ol>
        </div>
        <p className="text-xs text-slate-500 text-center">
          Las imágenes se transmiten encriptadas y no son almacenadas.
        </p>
        <button
  ...
  if (stage === "procesando") {
    return (
      <div className="flex flex-col items-center gap-4 py-10">
        ...
        <p className="text-slate-700 font-semibold">Verificando con RENAPER...</p>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <button
        onClick={() => { setStage("instrucciones"); setErrorMsg(null); setFrenteDni(null); setSelfie(null); }}
